...
Cypress.Commands.add(
  ...
  function (assetName, amount) {
    ...
    cy.get_asset_information().then(assets => {
      ...
      cy.exec(`curl -X POST -d '{"amount": "${amount}", "asset": "${asset.id}", "party": "${vegaWalletPublicKey}"}' -u "hedgehogandvega:hiccup" http://localhost:1790/api/v1/mint`)
      cy.get(vegaWalletContainer).within(() => {
        cy.get(vegaWalletCurrencyTitle, txTimeout).contains(asset.id, txTimeout);
      });
    });
  }
);

Cypress.Commands.add('vega_wallet_send_asset_to_reward_pool', (assetName, amount) => {
  cy.highlight(
